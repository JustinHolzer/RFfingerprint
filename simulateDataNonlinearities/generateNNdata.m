%% generateNNdata.m
%  This sythesizes data with QPSK modulation and introduces RF Impairments
%  to each of the transmitters.  
% This script saves the files in a format that can be read into the DNN
clear all
close all
flag_train = 1
num_Txrs = 14;
...
frameLength = 160;
if(flag_train)
    numTrainingFramesPerTxr = 5000;
else
    numTrainingFramesPerTxr = 10;
    %numTrainingFramesPerTxr = 10; % =10 if NN is already trained; = 5000 if used to train NN
end
training_frac = 0.8;
test_frac = 1 - training_frac;
filename = ['X_dataMatrix_c','.mat']  %save data  - ready for NN 

%X_training = zeros(frameLength,num_frames_per_txr*training_frac*numTxrs)
...
M = 4;
data = randi([0 (M - 1)],frameLength*numTrainingFramesPerTxr,1);  %total data (training + test)
...
y_all = ones(numTrainingFramesPerTxr,length(y));
for i = 1:length(y)
    y_all(:,i) = y(i);
end
y = y_all(:);
...
save(filename,'X','xTrainingFrames','xValFrames','xTestFrames','y','yTrain','yVal','yTest','phaseOffset','freqOffset','phNzFreqOff1', 'phNzFreqOff2','Ia', 'Ip', 'IDC', 'QDC', 'ampGain','frameLength','num_Txrs','numTrainingFramesPerTxr')
